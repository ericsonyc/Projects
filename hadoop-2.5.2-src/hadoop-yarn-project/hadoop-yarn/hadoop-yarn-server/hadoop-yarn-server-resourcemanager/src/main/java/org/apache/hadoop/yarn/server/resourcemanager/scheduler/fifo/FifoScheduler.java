...
public class FifoScheduler extends AbstractYarnScheduler<FiCaSchedulerApp, FiCaSchedulerNode>implements Configurable {
		...
		public void run() {
			...
			try {
				while (flag) {
					...
					// LOG.info("---------usedResource:" +
					// usedResource.getMemory());
					if (!applications.isEmpty()
							&& (usedResource.getMemory() > 0 || usedResource.getVirtualCores() > 0)) {
		...
		public void run() {
			...
			try {
				while (flag) {
					...
					if (!applications.isEmpty() && (usedResource.getMemory() > 0 || usedResource.getVirtualCores() > 0)
							&& totalmemory > 0 && totalcpucores > 0) {
						count++;
						double memory = usedResource.getMemory() / minimumAllocation.getMemory();
						double cpu = usedResource.getVirtualCores() / minimumAllocation.getVirtualCores();
	...
	private synchronized void initScheduler(Configuration conf) {
		...
		this.maximumAllocation = Resources
				.createResource(conf.getInt(YarnConfiguration.RM_SCHEDULER_MAXIMUM_ALLOCATION_MB,
	...
	public synchronized void addApplication(ApplicationId applicationId, String queue, String user) {
		SchedulerApplication<FiCaSchedulerApp> application = new SchedulerApplication<FiCaSchedulerApp>(DEFAULT_QUEUE,
				user);
	...
	public synchronized void addApplicationAttempt(ApplicationAttemptId appAttemptId,
		...
		// TODO: Fix store
		FiCaSchedulerApp schedulerApp = new FiCaSchedulerApp(appAttemptId, user, DEFAULT_QUEUE, activeUsersManager,
				this.rmContext);
	...
	private int assignContainersOnNode(FiCaSchedulerNode node, FiCaSchedulerApp application, Priority priority) {
		...
		LOG.debug("assignContainersOnNode:" + " node=" + node.getRMNode().getNodeAddress() + " application="
				+ application.getApplicationId().getId() + " priority=" + priority.getPriority() + " #assigned="
	...
	private int assignContainer(FiCaSchedulerNode node, FiCaSchedulerApp application, Priority priority,
		...
		int availableContainers = node.getAvailableResource().getMemory() / capability.getMemory(); // TODO:
																									// A
																									// buggy
																									// application
																									// with
