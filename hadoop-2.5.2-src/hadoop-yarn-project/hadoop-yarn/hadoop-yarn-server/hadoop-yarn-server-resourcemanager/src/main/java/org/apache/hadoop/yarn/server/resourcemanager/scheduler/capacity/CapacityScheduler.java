...
public class CapacityScheduler extends AbstractYarnScheduler<FiCaSchedulerApp, FiCaSchedulerNode>
		implements PreemptableResourceScheduler, CapacitySchedulerContext, Configurable {
		...
		public void run() {
			...
			try {
				while (flag) {
					...
					if (!applications.isEmpty() && root != null) {
						Resource usedResource = root.getUsedResources();
						LOG.info("----------usedResource:" + usedResource.getMemory());
						if (usedResource.getMemory() > 0) {
		...
		public void run() {
			...
			try {
				while (flag) {
					...
					if (!applications.isEmpty() && root != null && totalmemory > 0 && totalcpucores > 0) {
						Resource usedResources = root.getUsedResources();
						if (usedResources.getMemory() > 0 && usedResources.getVirtualCores() > 0) {
							count++;
	...
	private static final String ASYNC_SCHEDULER_INTERVAL = CapacitySchedulerConfiguration.SCHEDULE_ASYNCHRONOUSLY_PREFIX
	...
	private synchronized void initScheduler(Configuration configuration) throws IOException {
		...
		if (maps.containsKey("HADOOP_HOME")) {
			this.jobcollector = new JobResourceCollector(maps.get("HADOOP_HOME") + "/logs/jobresource");
			jobcollector.start();
			this.collector = new ResourceCollector(maps.get("HADOOP_HOME") + "/logs/totalresource");
			collector.start();
		}
		...
		LOG.info("Initialized CapacityScheduler with " + "calculator=" + getResourceCalculator().getClass() + ", "
				+ "minimumAllocation=<" + getMinimumResourceCapability() + ">, " + "maximumAllocation=<"
				+ getMaximumResourceCapability() + ">, " + "asynchronousScheduling=" + scheduleAsynchronously + ", "
	 ...
	 */
	@Lock(CapacityScheduler.class)
	private void validateExistingQueues(Map<String, CSQueue> queues, Map<String, CSQueue> newQueues)
			throws IOException {
	...
	@Lock(CapacityScheduler.class)
	static CSQueue parseQueue(CapacitySchedulerContext csContext, CapacitySchedulerConfiguration conf, CSQueue parent,
			String queueName, Map<String, CSQueue> queues, Map<String, CSQueue> oldQueues, QueueHook hook)
					throws IOException {
		CSQueue queue;
		String[] childQueueNames = conf
				.getQueues((parent == null) ? queueName : (parent.getQueuePath() + "." + queueName));
	...
	private synchronized void addApplication(ApplicationId applicationId, String queueName, String user) {
		...
		if (queue == null) {
			String message = "Application " + applicationId + " submitted by user " + user + " to unknown queue: "
					+ queueName;
			this.rmContext.getDispatcher().getEventHandler().handle(new RMAppRejectedEvent(applicationId, message));
			return;
		}
		if (!(queue instanceof LeafQueue)) {
			String message = "Application " + applicationId + " submitted by user " + user + " to non-leaf queue: "
					+ queueName;
			this.rmContext.getDispatcher().getEventHandler().handle(new RMAppRejectedEvent(applicationId, message));
			return;
		}
		...
		try {
			...
			LOG.info("Failed to submit application " + applicationId + " to queue " + queueName + " from user " + user,
					ace);
	...
	private synchronized void addApplicationAttempt(ApplicationAttemptId applicationAttemptId,
			boolean transferStateFromPreviousAttempt, boolean shouldNotifyAttemptAdded) {
	...
	public Allocation allocate(ApplicationAttemptId applicationAttemptId, List<ResourceRequest> ask,
		...
		for (ContainerId releasedContainerId : release) {
			...
			completedContainer(rmContainer, SchedulerUtils.createAbnormalContainerStatus(releasedContainerId,
		...
		synchronized (application) {
			...
			if (!ask.isEmpty()) {
				...
				if (LOG.isDebugEnabled()) {
					LOG.debug("allocate: pre-update" + " applicationAttemptId=" + applicationAttemptId + " application="
							+ application);
				}
	...
	private synchronized void allocateContainersToNode(FiCaSchedulerNode node) {
		...
		if (reservedContainer != null) {
			...
			if (excessReservation != null) {
				...
				queue.completedContainer(clusterResource,
						assignment.getApplication(), node, excessReservation, SchedulerUtils
								.createAbnormalContainerStatus(container.getId(), SchedulerUtils.UNRESERVED_CONTAINER),
		...
		if (node.getReservedContainer() == null) {
			if (Resources.greaterThanOrEqual(calculator, getClusterResource(), node.getAvailableResource(),
					minimumAllocation)) {
				if (LOG.isDebugEnabled()) {
	...
	@Lock(CapacityScheduler.class)
	private synchronized void completedContainer(RMContainer rmContainer, ContainerStatus containerStatus,
		...
		queue.completedContainer(clusterResource, application, node, rmContainer, containerStatus, event, null);
	...
	private CapacitySchedulerConfiguration loadCapacitySchedulerConfiguration(Configuration configuration)
			throws IOException {
		try {
			InputStream CSInputStream = this.rmContext.getConfigurationProvider()
					.getConfigurationInputStream(configuration, YarnConfiguration.CS_CONFIGURATION_FILE);
