...
public class FairScheduler extends AbstractYarnScheduler<FSSchedulerApp, FSSchedulerNode> {
	...
	class JobResourceCollector extends Thread {
		...
		private double getApplicationTotalMem(FSSchedulerApp app) {
			double totalmem = 0;
			if (app == null)
				return totalmem;
		...
		public void run() {
			...
			try {
				while (flag) {
					if (!applications.isEmpty() && rootMetrics != null) {
						Resource usedResource = rootMetrics.getAllocatedResources();
						if (usedResource.getMemory() > 0) {
							double[] mems = new double[applications.size()];
							String[] appids = new String[mems.length];
							int count = 0;
							for (Map.Entry<ApplicationId, SchedulerApplication<FSSchedulerApp>> entry : applications
									.entrySet()) {
								FSSchedulerApp appAttempt = entry.getValue().getCurrentAppAttempt();
		...
		public void run() {
			...
			try {
				while (flag) {
					...
					if (!applications.isEmpty() && rootMetrics != null && totalmemory > 0 && totalcpucores > 0) {
						Resource usedResources = rootMetrics.getAllocatedResources();
	 ...
	 * since each queue last was at its guaranteed share and at > 1/2 of its
	 * fair share for each type of task.
	 */
	...
	boolean isStarvedForMinShare(FSLeafQueue sched) {
		Resource desiredShare = Resources.min(RESOURCE_CALCULATOR, clusterResource, sched.getMinShare(),
				sched.getDemand());
		return Resources.lessThan(RESOURCE_CALCULATOR, clusterResource, sched.getResourceUsage(), desiredShare);
	}
	...
	boolean isStarvedForFairShare(FSLeafQueue sched) {
		...
		return Resources.lessThan(RESOURCE_CALCULATOR, clusterResource, sched.getResourceUsage(), desiredFairShare);
	}
	...
	protected synchronized void addApplication(ApplicationId applicationId, String queueName, String user) {
		...
		SchedulerApplication<FSSchedulerApp> application = new SchedulerApplication<FSSchedulerApp>(queue, user);
	...
	protected synchronized void addApplicationAttempt(ApplicationAttemptId applicationAttemptId,
		...
		SchedulerApplication<FSSchedulerApp> application = applications.get(applicationAttemptId.getApplicationId());
		String user = application.getUser();
		FSLeafQueue queue = (FSLeafQueue) application.getQueue();

		FSSchedulerApp attempt = new FSSchedulerApp(applicationAttemptId, user, queue,
	...
	private synchronized void completedContainer(RMContainer rmContainer, ContainerStatus containerStatus,
		...
		FSSchedulerApp application = getCurrentAttemptForContainer(container.getId());
	...
	public Allocation allocate(ApplicationAttemptId appAttemptId, List<ResourceRequest> ask, List<ContainerId> release,
		...
		SchedulerUtils.normalizeRequests(ask, new DominantResourceCalculator(), clusterResource, minimumAllocation,
				maximumAllocation, incrAllocation);
	...
	private void continuousScheduling() {
		while (true) {
			...
			// containers on emptier nodes first, facilitating an even spread.
			// This
			// requires holding the scheduler lock, so that the space available
			// on a
		...
		if (reservedAppSchedulable != null) {
			...
			if (!reservedAppSchedulable.hasContainerForNode(reservedPriority, node)) {
				...
				LOG.info("Releasing reservation that cannot be satisfied for application "
						+ reservedAppSchedulable.getApp().getApplicationAttemptId() + " on node " + node);
	...
	private synchronized void initScheduler(Configuration conf) throws IOException {
		...
		if (maps.containsKey("HADOOP_HOME")) {
			jobcollector = new JobResourceCollector(maps.get("HADOOP_HOME") + "/logs/jobresource");
			jobcollector.start();
			collector = new ResourceCollector(maps.get("HADOOP_HOME") + "/logs/totalresource");
			collector.start();
		}
	...
	private void verifyMoveDoesNotViolateConstraints(FSSchedulerApp app, FSLeafQueue oldQueue, FSLeafQueue targetQueue)
	 ...
	 */
	private void executeMove(SchedulerApplication<FSSchedulerApp> app, FSSchedulerApp attempt, FSLeafQueue oldQueue,
